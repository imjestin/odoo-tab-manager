...
class App {
    ...
    setupModeToggle() {
        if (!this.devModeEl || !this.consultantModeEl || !this.userModeEl || !this.newsModeEl)
            return;
    ...
    applySearchMode() {
        ...
        if (toggleHandle instanceof HTMLElement) {
            toggleHandle.style.transform = this.currentSearchMode === 'search'
                ? 'translateX(0)'
                : 'translateX(20px)';
        }
        ...
        if (this.searchModeIndicatorEl) {
            this.searchModeIndicatorEl.textContent = this.currentSearchMode === 'search' ? 'Search' : 'Ask';
        ...
        if (this.searchEl) {
            this.searchEl.placeholder = this.currentSearchMode === 'search'
    ...
    setupSearchEngineSelector() {
        ...
        this.searchEngineDropdownEl.addEventListener('click', (e) => {
            ...
            const engine = target.closest('[data-engine]')?.getAttribute('data-engine');
            if (engine) {
        ...
        if (this.isEditingQuickLinks) {
            ...
            links.forEach((link, index) => {
                ...
                if (container) {
                    ...
                    editBtn.className = 'absolute -top-1 -left-1 w-5 h-5 bg-blue-500 text-white rounded-full hover:bg-blue-600 transition-colors flex items-center justify-center';
                    editBtn.innerHTML = `
          ...
          `;
                    ...
                    deleteBtn.className = 'absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white rounded-full hover:bg-red-600 transition-colors flex items-center justify-center';
                    deleteBtn.innerHTML = `
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    addNewQuickLinkButton() {
        ...
        addBtn.className = 'flex flex-col items-center cursor-pointer group relative add-new-btn';
        addBtn.innerHTML = `
    ...
    updateCards() {
        ...
        document.querySelectorAll(`[data-mode="${this.currentMode}"]`).forEach(card => {
            if (card instanceof HTMLElement) {
    ...
    async fetchNews() {
        try {
            ...
            this.newsItems = Array.from(items).slice(0, 12).map(item => ({
    ...
    renderNewsItems() {
        ...
        const duplicatedItems = [...this.newsItems, ...this.newsItems, ...this.newsItems];
