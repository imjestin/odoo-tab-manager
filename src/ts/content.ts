...
function detectOdooWebsite(): boolean {
  ...
  const hasOdooGlobal = !!(window as any).odoo || !!(window as any).Odoo;
  const hasOpenerpGlobal = !!(window as any).openerp || !!(window as any).OpenERP;
...
function checkOdooMetadata() {
  const metadata = {
    ...
    customMeta: [] as Array<{name: string | null, property: string | null, content: string | null}>,
    scripts: [] as string[],
    links: [] as string[],
    forms: [] as string[],
    images: [] as string[],
...
function createFloatingButton() {
  ...
  const menu = floatingBtn.querySelector('.odoo-floating-menu') as HTMLElement;
  const menuItems = floatingBtn.querySelectorAll('.odoo-menu-item');
  const themeTabElement = document.getElementById('odoo-theme-tab') as HTMLElement;
  ...
  themeCapsules.forEach(capsule => {
    capsule.addEventListener('click', (e) => {
      ...
      if (theme) {
        const radio = document.getElementById(`theme-${theme}`) as HTMLInputElement;
        if (radio) {
          radio.checked = true;
  ...
  if (savedTheme) {
    ...
    const savedRadio = document.getElementById(`theme-${savedTheme}`) as HTMLInputElement;
    if (savedRadio) {
...
function executeOdooAction(action: string) {
...
// Apply Odoo theme
function applyOdooTheme(theme: string) {
  ...
  const selectedRadio = document.getElementById(`theme-${theme}`) as HTMLInputElement;
  if (selectedRadio) {
...
(function initQuickLogin() {
 function isLoginPage(): boolean {
  ...
  const isDatabasePage =
    window.location.pathname.includes('/web/database') ||
...
if (customUser && customPass) {
  ...
  `;
      ...
      for (const selector of possibleContainers) {
        ...
        if (loginContainer) {
          ...
          const parentStyle = loginContainer as HTMLElement;
  ...
  function login(username: string, password: string) {
    const loginInput = document.querySelector('input[name="login"], input[name="email"]') as HTMLInputElement;
