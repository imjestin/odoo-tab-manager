...
class PopupApp {
  private currentMode: any = 'developer';
  private currentPageInfo: PageInfo | null = null;

  // DOM Elements
  private currentModeEl: HTMLElement | null;
  private badgeTextEl: HTMLElement | null;
  private devBtn: HTMLElement | null;
  private consultantBtn: HTMLElement | null;
  private userBtn: HTMLElement | null;
  private newsBtn: HTMLElement | null;
  private addBookmarkBtn: HTMLElement | null;
  private bookmarksList: HTMLElement | null;
  private odooStatusEl: HTMLElement | null;
  private odooIndicatorEl: HTMLElement | null;
  private odooStatusTextEl: HTMLElement | null;
  private odooMetadataEl: HTMLElement | null;
  private odooGeneratorEl: HTMLElement | null;
  private odooGeneratorValueEl: HTMLElement | null;
  private odooDescriptionEl: HTMLElement | null;
  private odooDescriptionValueEl: HTMLElement | null;
  private odooKeywordsEl: HTMLElement | null;
  private odooKeywordsValueEl: HTMLElement | null;
  private odooAuthorEl: HTMLElement | null;
  private odooAuthorValueEl: HTMLElement | null;
  private odooScriptsEl: HTMLElement | null;
  private odooScriptsValueEl: HTMLElement | null;
  private odooLinksEl: HTMLElement | null;
  private odooLinksValueEl: HTMLElement | null;

  // Mode display mappings
  private readonly modeDisplay: Record<string, string> = {
    developer: 'Developer',
  ...
  private readonly badgeText: Record<string, string> = {
    developer: 'DEV',
    consultant: 'CON',
    user: 'USER',
    news: 'NEWS'
  ...
  constructor() {
    ...
    // Load current mode
    this.currentMode = (localStorage.getItem('odooMode') as any) || 'developer';
  ...
  private updateBrowserAction(mode: any): void {
    if (chrome.action) {
  ...
  private updateButtonStates(): void {
    ...
    const modes: string[] = ['developer', 'consultant', 'user', 'news'];
  ...
  private switchMode(mode: any): void {
  ...
  private createBookmarkElement(bookmark: Bookmark, index: number): HTMLElement {
  ...
  private addCurrentBookmark(): void {
    ...
    chrome.tabs.query({active: true, currentWindow: true}, (tabs) => {
      ...
      chrome.scripting.executeScript({
        ...
        chrome.tabs.sendMessage(activeTab.id!, {action: 'getCurrentPageInfo'}, (response: PageInfo) => {
          if (chrome.runtime.lastError) {
            console.error('Error sending message to content script:', chrome.runtime.lastError);
            // Fallback: use tab information directly
            if (activeTab.url && activeTab.title) {
              const fallbackInfo: PageInfo = {
                url: activeTab.url,
  ...
  private saveBookmark(pageInfo: PageInfo): void {
  ...
  private showMessage(message: string, type: 'info' | 'error' | 'warning' | 'success' = 'info'): void {
    ...
    messageEl.className = `text-sm p-3 rounded-lg mb-3 border ${
      type === 'error' ? 'bg-red-50 text-red-700 border-red-200 dark:bg-red-900/20 dark:text-red-400 dark:border-red-800' :
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      if (tabs[0] && tabs[0].id) {
        ...
        const sendMessagePromise = new Promise((resolve, reject) => {
          ...
          chrome.tabs.sendMessage(tabs[0].id, { action: 'getCurrentPageInfo' }, (response: any) => {
            if (chrome.runtime.lastError) {
        ...
        sendMessagePromise.then((response: any) => {
  ...
  private performBasicOdooDetection(url: string, title: string): void {
  ...
  private isOdooWebsite(url: string, title: string): boolean {
    try {
  ...
  private updateOdooStatus(isOdoo: boolean, message: string, metadata?: any): void {
    if (!this.odooIndicatorEl || !this.odooStatusTextEl || !this.odooStatusEl) return;
  ...
  private displayOdooMetadata(metadata: any): void {
    if (!this.odooMetadataEl) return;
